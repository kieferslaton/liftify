...
router.route('/signup').post( async (req, res) => {
    ...
    newUser.save().then(() => {
        const token = jwt.sign({ userId: newUser._id }, 'expelliarmus', {expiresIn: '7d'})
...
router.route('/login').post( async (req, res) => {
        ...
        if(passwordsMatch) {
            const token = jwt.sign({userId: user._id}, 'expelliarmus', {expiresIn: '7d'})
            res.status(200).json(token)
        } else {
            res.status(401).json()
        }
...
router.route('/accounts').post(async (req, res) => {
    ...
    const {userId} = jwt.verify(auth, 'expelliarmus')

    if(!userId) {
        return res.status(401).send('User id failed')
    }
